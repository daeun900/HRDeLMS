...
include "../../include/include_function.php";
	...
	*/
	...
	$configPath = $HomeDirectory"/lib/LGU_XPay_Crossplatform_PHP/lgdacom"; //LG유플러스에서 제공한 환경파일("/conf/lgdacom.conf,/conf/mall.conf") 위치 지정. 
    ...
    if ($xpay->TX()) {
		...
		*/
        ...
        if( "0000" == $xpay->Response_Code() ) {
			...
			$PayMethod = $xpay->Response("LGD_PAYTYPE",0);
			$MOID = $xpay->Response("LGD_OID",0);
			$tid = $xpay->Response("LGD_TID",0);
			$merchantData_array = explode("|",$xpay->Response("LGD_PRODUCTINFO",0));
			$PaymentSheet_idx = $merchantData_array[1];
			$LectureStart = $merchantData_array[2];
			$LectureEnd = $merchantData_array[3];

			$Sql = "UPDATE PaymentSheet2 SET PayStatus='Y', PayMethod='$PayMethod', MOID='$MOID', tid='$tid', PayDate=NOW() WHERE idx=$PaymentSheet_idx";
			//echo $Sql."<BR><BR><BR>";
			$Row = mysql_query($Sql);
...
<!--
	alert("<?=$msg?>");
	top.location.href="/mypage/manager_payment2.php";
//-->
</script>