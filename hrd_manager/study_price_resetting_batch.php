...
else $where[] = "a.ServiceType IN (1,3,5,9)";

if($PackageYN) {
    if($PackageYN=="Y")    $where[] = "a.PackageRef>0";
    if($PackageYN=="N")    $where[] = "a.PackageRef<1";
}
...
$Colume = "a.Seq, a.ServiceType, a.ID, a.LectureStart, a.LectureEnd, a.Progress, a.MidStatus, a.MidSaveTime, a.MidScore, a.TestStatus, a.TestScore, a.TestSaveTime, a.ReportStatus, a.ReportSaveTime,
		a.ReportScore, a.TotalScore, a.PassOK, a.certCount, a.StudyEnd, a.PackageRef, a.PackageGroupNo, a.OpenChapter, a.Tutor, a.Price, a.rPrice, a.rPrice2, 
...
function ChangeBatch() {
	...
	var ServiceTypeApply = $("#ServiceTypeApply").val();
...
function BatchProcess(i) {
	...
	if(i<totalcount) {
		...
		if ($("input:checkbox[id='check_seq_"+i+"']").is(":checked") == false){
			...
			$("div[id='status']:eq("+i+")").load('./study_price_resetting_batch_process.php',
			{ 'Seq': $("input:checkbox[id='check_seq_"+i+"']").val(),
				'ServiceTypeApply': $("#ServiceTypeApply").val()
	...
	<div id="wrap">
		<div class="Content">
	        <div class="contentBody">
        	    ...
        	    <div class="conZone">
                	<div class="tl pt5">
    					<span class="fs14 fc333"><img src="images/sub_title.gif" align="absmiddle"> 교육비 재설정 대상 | 회사규모 변경 또는 컨텐츠에서 교육비가 변경된 경우 교육비 재설정을 진행합니다.</span>
    				</div>
       				<P style="text-align:right">
    				<select name="ServiceTypeApply" id="ServiceTypeApply" style="height:35px">
    					<option value="">-- 비환급과정 교육비 적용 여부 --</option>
    					<option value="Y">비환급과정 교육비 적용(환급비,자부담금을 컨텐츠에 등록된 금액 적용)</option>
    					<option value="N">비환급과정 교육비 미적용(환급비,자부담금 0원으로 설정)</option>
    				</select>&nbsp;&nbsp;
