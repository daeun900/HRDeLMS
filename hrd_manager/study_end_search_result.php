...
<table width="100%" cellpadding="0" cellspacing="0" class="list_ty01 gapT20">
    ...
    <tr>
        ...
        <!-- <td></td>  -->
        <td>
        	<!-- 241028.YEON. 대량데이터수정시 과부화로인해 주석처리 -->
    ...
    <tr>
        ...
        <? if($LectureIsValid) {
            if($ServiceType == 1){ ?>
			    ...
			    <button type="button" name="CertBtn08" id="CertBtn08" class="btn round btn_LGray line" style="margin-bottom:5px;" onclick="StudyEndDocument02Mail('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','<?=$ServiceType?>','<?=$LectureCode?>')">교육진행보고서 메일발송</button><br>
            <?} ?>
        ...
        <?if(($LectureIsValid &&($ServiceType == 1) && ($StudyCount>0))){?>
            <button type="button" name="CertBtn09" id="CertBtn10" class="btn round btn_LGray line" style="font-size:14px;border: none;background: none;" onclick="StudyEndDocumentPoll('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','<?=$LectureCode?>');">설문 결과</button><br>
        <?}?>
        	<!-- 인증기준에 해당되지 않는다는 전제하에 숨김처리. 240524.YEON[요청자:김지혜주임] -->
        	<!-- <button style="font-size:14px; min-width:5px; min-height:30px; border: none; background: none;" type="button" name="CertBtn08" id="CertBtn08" class="btn round btn_LGray line" onclick="StudyEndDocument('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','<?=$ServiceType?>','<?=$LectureCode?>')">결과</button>  -->
        </td>
    </tr>
    <?
                   $preCode = $CompanyCode;
