...
foreach($LoginAdminSubMenuGrant_array as $LoginAdminSubMenuGrant_array_value) {
    ...
    if(substr($LoginAdminSubMenuGrant_array_value,0,1)=="H") {
        $Top_Link_H[] = $LoginAdminSubMenuGrant_array_value;
    }
    if(substr($LoginAdminSubMenuGrant_array_value,0,1)=="I") {
        $Top_Link_I[] = $LoginAdminSubMenuGrant_array_value;
    }
    if(substr($LoginAdminSubMenuGrant_array_value,0,1)=="J") {
        $Top_Link_J[] = $LoginAdminSubMenuGrant_array_value;
    }
}
...
$Top_Link_H_page = $Manager_Top_Link_array[$Top_Link_H[0]];
$Top_Link_I_page = $Manager_Top_Link_array[$Top_Link_I[0]];
$Top_Link_J_page = $Manager_Top_Link_array[$Top_Link_J[0]];
?>
...
<link rel="stylesheet" href="./css/style.css?v240527" type="text/css">
...
<SCRIPT LANGUAGE="JavaScript">
    ...
    function changeSelect2Style(){
    	$(".select2-container .select2-selection--single").attr("style","height:36px !important; padding:4px 0px; border:1px solid #c6c6c6; border-radius:3px;");
		...
		<div class="inner">
			...
			<div class="mainMenu">
				<ul class="area">
					...
					<li><a href="<?=$Top_Link_E_page?>" <?if($MenuType=="E") {?>class="show"<?}?>>사업주훈련</a></li>
					<?}?>
					<?if(in_array('F',$LoginAdminTopMenuGrant_array)){?>
					<li><a href="<?=$Top_Link_F_page?>" <?if($MenuType=="F") {?>class="show"<?}?>>내일배움카드</a></li>
					<?}?>
					<?if(in_array('G',$LoginAdminTopMenuGrant_array)){?>
					<li><a href="<?=$Top_Link_G_page?>" <?if($MenuType=="G") {?>class="show"<?}?>>FLEX</a></li>
					<?}?>
					<?if(in_array('H',$LoginAdminTopMenuGrant_array)){?>
					<li><a href="<?=$Top_Link_H_page?>" <?if($MenuType=="H") {?>class="show"<?}?>>커뮤니티관리</a></li>
					<?}?>
					<?if(in_array('I',$LoginAdminTopMenuGrant_array)){?>
					<li><a href="<?=$Top_Link_I_page?>" <?if($MenuType=="I") {?>class="show"<?}?>>통계관리</a></li>
					<?}?>
					<?if(in_array('J',$LoginAdminTopMenuGrant_array)){?>
					<li><a href="<?=$Top_Link_J_page?>" <?if($MenuType=="J") {?>class="show"<?}?>>사이트관리</a></li>
					<?}?>
				</ul>
			</div>
			<!-- menu // -->
