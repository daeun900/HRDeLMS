...
class Page
{
	...
	function blockList()
	{
		...
		if($this->block != 1)
		{
			...
			$block_str .= '<li><a href="javascript:pageRun(' . $temp . ');" title="이전 ' . $this->block_size . '" class="prev">◀</a>&nbsp;</li>';
		}
		else
		{
			$block_str .= '';
		...
		for($i = 0; $i < count($arrBlock); $i++)
		{
			...
			{
				$block_str .= '<li><a href="javascript:pageRun('. $arrBlock[$i] . ');" class="active"><strong>' . $arrBlock[$i] . '</strong></a></li>';
			}
		...
		if($this->block != $this->block_count && $this->tot_no != 0){
			...
			$block_str .= '<li>&nbsp;<a href="javascript:pageRun(' . $temp . ')" title="다음 ' . $this->block_size . '" class="next">▶</a></li>';
		}
		else
		{
			$block_str .= '';
