...
$objPHPExcel->getActiveSheet()->getCell('D1')->setValue("이메일");
...
$i=2;
$k = 1;
$SQL = "SELECT a.*, b.Name, c.CompanyName, c.CompanyCode FROM EmailSendLog AS a LEFT OUTER JOIN Member AS b ON a.ID=b.ID LEFT OUTER JOIN Company AS c ON b.CompanyCode=c.CompanyCode  $where $orderby";
...
while($ROW = mysqli_fetch_array($QUERY))
	...
	$objPHPExcel->getActiveSheet()->getCell('D'.$i)->setValueExplicit($Email, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->getCell('E'.$i)->setValueExplicit($MassageTitle, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->getCell('F'.$i)->setValueExplicit($RegDate, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->getCell('G'.$i)->setValueExplicit($Email_ReturnCode_array[$Code], PHPExcel_Cell_DataType::TYPE_STRING);
