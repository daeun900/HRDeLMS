...
$StudyPeriod  = Replace_Check($StudyPeriod); //수강기간
...
$TutorStatus  = Replace_Check($TutorStatus); //첨삭정렬
...
<table width="100%" cellpadding="0" cellspacing="0" class="list_ty01 gapT20">
	...
	<tr>
		...
		<td align="center" bgcolor="#FFFFFF" class="text01">
			<a href="Javascript:CourseInfo('<?=$LectureCode?>');"><?=$ContentsName?></a><br /><?=$LectureStart?> ~ <?=$LectureEnd?><br />첨삭완료 : <?=date("Y-m-d", $Tutor_limit_day)?>까지
		</td>
		...
		<?if($AdminWrite=="Y") {?>
			<?if($StudyEnd=="N") {?>
			<?if($MidStatus=="C" || $MidStatus=="Y") {?><button type="button" name="MidResetBtn" id="MidResetBtn" class="btn round btn_LBlue line" onclick="MidResetProcess('<?=$Name?>','<?=$Seq?>')">중간</button><?}?>
			<?if($TestStatus=="C" || $TestStatus=="Y") {?><br /><button type="button" name="TestResetBtn" id="TestResetBtn" class="btn round btn_LBlue line" onclick="TestResetProcess('<?=$Name?>','<?=$Seq?>')">최종</button><?}?>
			<?if($ReportStatus=="C" || $ReportStatus=="Y") {?><br /><button type="button" name="ReportResetBtn" id="ReportResetBtn" class="btn round btn_LBlue line" onclick="ReportResetProcess('<?=$Name?>','<?=$Seq?>')">과제</button><?}?>
			<?}?>
		<?}?>
		</td>
		...
		<td align="center" bgcolor="#FFFFFF" class="text01"><?=$StudyEndView?></td>
	</tr>
	<?
		}
	}else{
