...
try{
	...
	for($i = $startLine; $i <= $maxRow; $i++) {
		if($excelType == 'v2'){
			...
			$Price = trim($objWorksheet->getCell('M' . $i)->getValue()); // 수강료
			$rPrice = trim($objWorksheet->getCell('N' . $i)->getValue()); // 환급비
			$ServiceType = trim($objWorksheet->getCell('O' . $i)->getValue()); // 서비스 구분
			$UserID = trim($objWorksheet->getCell('P' . $i)->getValue()); // 수강생 아이디
			$Pwd = trim($objWorksheet->getCell('Q' . $i)->getValue()); // 비밀번호
			$nwIno = trim($objWorksheet->getCell('R' . $i)->getValue()); // 비용수급사업장
			$trneeSe = trim($objWorksheet->getCell('S' . $i)->getValue()); // 훈련생구분
			$IrglbrSe = trim($objWorksheet->getCell('T' . $i)->getValue()); // 비정규직구분
			$OpenChapter = trim($objWorksheet->getCell('U' . $i)->getValue()); // 실시회차
			$SalesID = trim($objWorksheet->getCell('V' . $i)->getValue()); // 영업담당자 아이디
			$EduManager = trim($objWorksheet->getCell('W' . $i)->getValue()); // 교육담당자 여부
			$tok2ID = trim($objWorksheet->getCell('X' . $i)->getValue()); // tok2 아이디
