...
if($mode!="new") {
	...
	if($ChapterType=="A") {
		...
		$ChapterType_disbled[4] = "disabled";
	}
	if($ChapterType=="B") {
    			...
    			<table width="100%" cellpadding="0" cellspacing="0" class="view_ty01">
			  		...
			  		<tr id="ExamTR" style="display:<?=$ExamTR?>">
						...
						<td>
            				...
            				<?if($mode=="new") {?>&nbsp;&nbsp;<input type="button" value="선택한 [차시 구분] 모두 추가하기" class="btn_inputSm01" onclick="ChapterExamBatch();"><?}?>
						</td>
			  		</tr>
			  		<tr id="ExamTR" style="display:<?=$ExamTR?>">
        				<th>문제은행 선택</th>
        				...
        				<td>
            				...
            				<table id="ExamTable" border="0" cellspacing="0" width="870px" style="border-top: #a2a2a2 1px solid; border-right: #a2a2a2 1px solid; border-bottom: #a2a2a2 1px solid; border-left: #a2a2a2 1px solid; background-color:#ffffff">
            					...
            					if($mode!="new") {
            						...
            						<tr>
            							<td align="center">
            								<input type="hidden" name="Exam_idx_temp" id="Exam_idx_temp" value="<?=$Row['idx']?>" readonly>
            								<input type="text" name="Exam_idx_text" style="width:100%" value="<?=$i?>. [<?=$ExamType_array[$Row['ExamType']]?>] <?=html_quote($Row['Question'])?>" readonly>
            							</td>
            							<td align="center">
            								<input type="button" value="삭제" onclick="Javascript:ChapterExamDelRow(this);" class="btn_inputSm01">
            							</td>
            						</tr>
            						<?
            							$i++;
			  		...
			  		<tr id="DiscussionTR" style="display:<?=$DiscussionTR?>">
						<th>과목 구분 선택</th>
        				<td>
            				<select name="SubjectGubun" id="SubjectGubun" onchange="ChapterContentsSelect('<?=$Sub_idx?>');" style="width:620px">
            					<option value="">-- 과목구분 선택 --</option>
            					<?
            					$SQL = "SELECT * FROM DiscussionTopic WHERE Del='N' $ContentGubun_str ORDER BY Gubun ASC";
            					$QUERY = mysqli_query($connect, $SQL);
            					if($QUERY && mysqli_num_rows($QUERY)){
            						while($Row = mysqli_fetch_array($QUERY)){
            					?>
            					<option value="<?=$Row['idx']?>" <?if($Row['idx']==$Sub_idx) {?>selected<?}?>><?=$Row['Gubun']?> / <?=$Row['Topic']?></option>
            					<?
            						}
            					}
            					?>
