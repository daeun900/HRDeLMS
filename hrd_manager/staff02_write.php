...
if($mode!="new") {
	...
	if($Row) {
		...
		$CommissionRatio = $Row['CommissionRatio']; //영업자 수수료율
...
?>
                ...
                <!-- 입력 -->
				<form name="Form1" method="post" action="staff02_script.php" target="ScriptFrame">
                ...
                <table width="100%" cellpadding="0" cellspacing="0" class="view_ty01">
                  ...
                  <tr>
                    <th>카테고리</th>
                    <td><span id="DeptCategorySelected"></span>&nbsp;<button type="button" onclick="DeptCategorySelect('B');" class="btn round btn_LBlue line"><i class="xi-folder-open"></i> 카테고리 찾기</button>
                  </tr>
				  <?if($mode=="new") {?>
				  <tr>
                    <th>아이디</th>
				  ...
				  <tr>
                    <th>영업자 수수료율</th>
                    <td><input name="CommissionRatio" type="text"  size="10" value="<?=$CommissionRatio?>"> %</td>
                  </tr>
				  <tr>
                    <th>사용 여부</th>
...
function SubmitOk() {
	...
	if(val.CommissionRatio.value=="") {
		alert("영업자 수수료율을 입력하세요.");
		val.CommissionRatio.focus();
		return;
	}
	if(IsNumber(val.CommissionRatio.value)==false) {
		alert("영업자 수수료율은 숫자만(정수) 입력하세요.");
		val.CommissionRatio.focus();
		return;
	}
