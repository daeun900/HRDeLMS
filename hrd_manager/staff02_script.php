...
$CommissionRatio = Replace_Check($CommissionRatio);
...
if($mode=="new") { //새글 작성---------------------------------------------------------------------------------------------------------
	...
	$Sql = "INSERT INTO StaffInfo 
				(idx, ID, Dept_idx, Dept, Name, Pwd, Team, Mobile, Phone, Email, BankName, BankNumber, UseYN, Del, RegDate, CommissionRatio, SiteCode) 
				VALUES ($maxno, '$ID', $Dept_idx, '$Dept', '$Name', password('$Pwd'), '$Team', '$Mobile', '$Phone', '$Email', '$BankName', '$BankNumber', '$UseYN', '$Del', NOW(), $CommissionRatio, '$SiteCode')";
...
if($mode=="edit") { //글 수정---------------------------------------------------------------------------------------------------------
	...
	$Sql = "UPDATE StaffInfo SET Dept_idx=$Dept_idx, Name='$Name', Team='$Team', Mobile='$Mobile', Phone='$Phone', Email='$Email', BankName='$BankName', BankNumber='$BankNumber', UseYN='$UseYN', CommissionRatio=$CommissionRatio WHERE idx=$idx";
	$Row = mysqli_query($connect, $Sql);

	$cmd = true;
	$url = "staff02_read.php?idx=".$idx;
