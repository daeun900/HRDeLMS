...
$Category1 = Replace_Check($CategoryA); //과정분류 대분류
$Category2 = Replace_Check($CategoryB); //과정분류 소분류
$Category3 = Replace_Check($CategoryC); //과정분류 대분류(직무스킬업) 세부분류
...
$EmpRate = Replace_Check($EmpRate); //취업률
$Price = Replace_Check($Price); //교육비용 일반
$Price01View = Replace_Check($Price01View); //교육비용 환급비용 - 일반훈련생 
$Price02View = Replace_Check($Price02View); //교육비용 환급비용 - 취성패
$Price03View = Replace_Check($Price03View); //교육비용 환급비용 - 근로장려금 
$Price01 = Replace_Check($Price01); //자부담금 일반훈련생 
$Price02 = Replace_Check($Price02); //자부담금 취성패
$Price03 = Replace_Check($Price03); //자부담금 근로장려금
...
$ctype = Replace_Check($ctype); //구분 A:사업자과정, B:근로자과정
...
$Keyword = Replace_Check($Keyword); //키워드
...
if($mode=="new") { //새글 작성---------------------------------------------------------------------------------------------------------
	...
	$maxno = max_number("idx","CourseCyber");

	$Sql = "INSERT INTO CourseCyber
				(idx, ClassGrade, LectureCode, UseYN, PassCode, HrdCode, Category1, Category2, Category3, ServiceType, ContentsName, CompleteTime, ProgressCheck, 
				Chapter, ContentsTime, EmpRate, Price, Price01, Price02, Price03, Price01View, Price02View, Price03View, Professor, Limited, ContentsPeriod, ContentsAccredit, ContentsExpire, Cp, Commission, 
				...
				PassReport, PassScore, Intro, EduTarget, EduGoal, Hit, Del, RegDate, PackageYN, PackageRef, PackageLectureCode, ChapterLimit, tok2, ctype, IE8Compat, ContentsURLSelect, Keyword)
				VALUES 
				($maxno, '$ClassGrade', '$LectureCode', '$UseYN', '$PassCode', '$HrdCode', '$Category1', '$Category2', '$Category3', '$ServiceType', '$ContentsName', $CompleteTime, '$ProgressCheck', 
				'$Chapter', '$ContentsTime', $EmpRate, $Price, $Price01, $Price02, $Price03, $Price01View, $Price02View, $Price03View, '$Professor', $Limited, '$ContentsPeriod', '$ContentsAccredit', '$ContentsExpire', '$Cp', $Commission, 
				'$Mobile', $BookPrice, '$BookIntro', '$attachFile', '$PreviewImage', '$BookImage', $Mid01EA, $Mid01Score, $Mid02EA, $Mid02Score, $Mid03EA, $Mid03Score, $Test01EA, $Test01Score, $Test02EA, 
				$Test02Score, $Test03EA, 
				$Test03Score, $Report01EA, $Report01Score, $Report02EA, $Report02Score, $Report03EA, $Report03Score, $TestTime, $MidRate, $TestRate, $ReportRate, $PassProgress, $TotalPassMid, $TotalPassTest, $PassTest, $TotalPassReport, 
				$PassReport, $PassScore, '$Intro', '$EduTarget', '$EduGoal', '$Hit', 'N', NOW(), 'N',0,'', '$ChapterLimit', '$tok2', '$ctype', '$IE8Compat', '$ContentsURLSelect', '$Keyword')";
	$Row = mysqli_query($connect, $Sql);
	
	$cmd = true;
	$url = "course_cyber_read.php?idx=".$maxno;

} //새글 작성-------------------------------------------------------------------------------------------------------------------------

if($mode=="edit") { //글 수정---------------------------------------------------------------------------------------------------------
	$Sql = "UPDATE CourseCyber SET 
					ClassGrade='$ClassGrade', UseYN='$UseYN', PassCode='$PassCode', HrdCode='$HrdCode', Category1='$Category1', Category2='$Category2', Category3='$Category3', ServiceType='$ServiceType', 
					ContentsName='$ContentsName', CompleteTime=$CompleteTime, ProgressCheck='$ProgressCheck', Chapter='$Chapter', ContentsTime='$ContentsTime', EmpRate = $EmpRate, Price=$Price, 
					Price01=$Price01, Price02=$Price02, Price03=$Price03, Price01View=$Price01View, Price02View=$Price02View, Price03View=$Price03View, Professor='$Professor', Limited=$Limited,
					...
					Test01Score=$Test01Score, Test02EA=$Test02EA, Test02Score=$Test02Score, Test03EA=$Test03EA, Test03Score=$Test03Score,
                    Report01EA=$Report01EA, Report01Score=$Report01Score, Report02EA=$Report02EA, Report02Score=$Report02Score, Report03EA=$Report03EA, Report03Score=$Report03Score, TestTime=$TestTime, MidRate=$MidRate, 
					...
					TotalPassReport=$TotalPassReport, PassReport=$PassReport, PassScore=$PassScore, Intro='$Intro', EduTarget='$EduTarget', EduGoal='$EduGoal', Hit='$Hit', ChapterLimit='$ChapterLimit',
                    tok2='$tok2', IE8Compat='$IE8Compat', ContentsURLSelect='$ContentsURLSelect' , Keyword='$Keyword'  
			WHERE idx=$idx AND LectureCode='$LectureCode'";
	$Row = mysqli_query($connect, $Sql);
	
	$cmd = true;
	$url = "course_cyber_read.php?idx=".$idx;

} //글 수정-------------------------------------------------------------------------------------------------------------------------

if($mode=="del") { //글 삭제---------------------------------------------------------------------------------------------------------

	$Sql = "UPDATE CourseCyber SET Del='Y' WHERE idx=$idx AND LectureCode='$LectureCode'";
	$Row = mysqli_query($connect, $Sql);

	$cmd = true;
	$url = "course_cyber.php?col=".$col."&sw=".$sw;
