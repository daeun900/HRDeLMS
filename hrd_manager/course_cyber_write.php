<?
$MenuType = "F";
$PageName = "course_cyber";
$ReadPage = "course_cyber_read";
?>
...
if($mode!="new") {
	...
	$Sql = "SELECT * FROM CourseCyber WHERE idx=$idx";
	...
	if($Row) {
		...
		$Category3 = $Row['Category3']; //과정분류 대분류(직무스킬업) 세부분류
		...
		$EmpRate = $Row['EmpRate']; //취업률
		$Price = $Row['Price']; //교육비용 일반
		$Price01 = $Row['Price01']; //자부담금 일반훈련생
		$Price02 = $Row['Price02']; //자부담금 취성패
		$Price03 = $Row['Price03']; //자부담금 근로장려금
		$Price01View = $Row['Price01View']; //교육비용 환급비용 - 일반훈련생
		$Price02View = $Row['Price02View']; //교육비용 환급비용 - 취성패
		$Price03View = $Row['Price03View']; //교육비용 환급비용 - 근로장려금
...
$(document).ready(function(){
	//"직무스킬업"이 선택되어있으면, 하위분류 show
	if($("#Category1_03").is(":checked") == true){
	    $("#Category3Span").show();
	}
...
<script>
$(function(){	
	$("#Category1_03").click(function(e){
		if($("#Category1_03").is(":checked") == true){
			//"직무스킬업"선택 시, 하위분류 SHOW
		    $("#Category3Span").show();
		}else{
			//"직무스킬업" 미선택 시, 하위분류 HIDE
			$("#Category3Span").hide();

			//하위분류 선택되어있다면, 체크해
			$("input:checkbox[id='Category3_01']").prop("checked", false);
			$("input:checkbox[id='Category3_02']").prop("checked", false);
			$("input:checkbox[id='Category3_03']").prop("checked", false);
			$("input:checkbox[id='Category3_04']").prop("checked", false);
		}
    });
})
</script>
				...
				<table width="100%"  border="0" cellspacing="0" cellpadding="0" style="margin-bottom:10px;">
					<tr>
						<td align="right"><input type="button" value="컨텐츠 정보 가져오기" onclick="CourseCyberCopy()" class="btn_inputBlue01"></td>
					</tr>
				</table>
				<?}?>
				<form name="Form1" method="post" action="course_cyber_script.php" target="ScriptFrame">
    				<INPUT TYPE="hidden" name="mode" value="<?=$mode?>">
    				<INPUT TYPE="hidden" name="idx" value="<?=$idx?>">
    				<INPUT TYPE="hidden" name="ctype" value="B">
    				<INPUT TYPE="hidden" NAME="CategoryA" id="CategoryA" value="<?=$Category1?>">
    				<INPUT TYPE="hidden" NAME="CategoryB" id="CategoryB" value="<?=$Category2?>">
    				<INPUT TYPE="hidden" NAME="CategoryC" id="CategoryC" value="<?=$Category3?>">
                    <table width="100%" cellpadding="0" cellspacing="0" class="view_ty01">
        				...
        				<tr>
        					...
        					<td align="left"colspan="3">
        						<input type="checkbox" name="Category1" id="Category1_01" value="1" <?if(in_array('1',$Category1_array)){?>checked<?}?>> <label for="Category1_01">국민내일배움카드</label>&nbsp;&nbsp;&nbsp;&nbsp;
        						<input type="checkbox" name="Category1" id="Category1_02" value="2" <?if(in_array('2',$Category1_array)){?>checked<?}?>> <label for="Category1_02">평생교육바우처</label>&nbsp;&nbsp;&nbsp;&nbsp;
        						<input type="checkbox" name="Category1" id="Category1_03" value="3" <?if(in_array('3',$Category1_array)){?>checked<?}?>> <label for="Category1_03">직무스킬업</label>&nbsp;&nbsp;&nbsp;&nbsp;
        						<span style="display:none;" id="Category3Span">(
            						<input type="checkbox" name="Category3" id="Category3_01" value="1" <?if(in_array('1',$Category3_array)){?>checked<?}?>> <label for="Category3_01">NCS직업교육</label>&nbsp;&nbsp;&nbsp;&nbsp;
            						<input type="checkbox" name="Category3" id="Category3_02" value="2" <?if(in_array('2',$Category3_array)){?>checked<?}?>> <label for="Category3_02">직무핵심스킬</label>&nbsp;&nbsp;&nbsp;&nbsp;
            						<input type="checkbox" name="Category3" id="Category3_03" value="3" <?if(in_array('3',$Category3_array)){?>checked<?}?>> <label for="Category3_03">OA달인</label>&nbsp;&nbsp;&nbsp;&nbsp;
            						<input type="checkbox" name="Category3" id="Category3_04" value="4" <?if(in_array('4',$Category3_array)){?>checked<?}?>> <label for="Category3_04">자격증</label>&nbsp;&nbsp;&nbsp;&nbsp;
        						)</span>
							</td>
						</tr>
						<tr>
        					<th>과정 중분류</th>
        					<td align="left"colspan="3">
        						<input type="checkbox" name="Category2" id="Category2_01" value="1" <?if(in_array('1',$Category2_array)){?>checked<?}?>> <label for="Category2_01">NCS직업교육</label>&nbsp;&nbsp;&nbsp;&nbsp;
        						<input type="checkbox" name="Category2" id="Category2_02" value="2" <?if(in_array('2',$Category2_array)){?>checked<?}?>> <label for="Category2_02">일반사무직</label>&nbsp;&nbsp;&nbsp;&nbsp;
        						<input type="checkbox" name="Category2" id="Category2_03" value="3" <?if(in_array('3',$Category2_array)){?>checked<?}?>> <label for="Category2_03">IT∙4차산업</label>&nbsp;&nbsp;&nbsp;&nbsp;
        						<input type="checkbox" name="Category2" id="Category2_04" value="4" <?if(in_array('4',$Category2_array)){?>checked<?}?>> <label for="Category2_04">병원</label>&nbsp;&nbsp;&nbsp;&nbsp;
							</td>
						</tr>
						<tr>
        					<th>키워드(과정 소분류)</th>
        					<td align="left" colspan="3">
        						<span class="redB">키워드 입력 시, 키워드 앞에 # 을 붙여주세요. 키워드는 최대3개만 보여집니다. <br> (ex)#사이버연수원 #교육원 #내일배움카드</span><br>
        						<input name="Keyword" id="Keyword" type="text"  size="150" value="<?=$Keyword?>" maxlength="300">
        					</td>
						</tr>
        				<tr>
							<th>서비스 구분</th>
        				...
        				<tr>
        					...
        					<td bgcolor="#FFFFFF" colspan="3"> 
            					<input name="EmpRate" id="EmpRate" type="text"  size="10" value="<?=$EmpRate?>" maxlength="7" style="text-align:right" oninput="rateChange()"> %
        					</td>
        				</tr>
        				<tr>
        					<th>교육비용</th>
        				...
        				<tr>
        					<th>자부담금</th>
        					<td bgcolor="#FFFFFF" colspan="3"> 
            					<?=$CompanyScaleTitle01?> : <input name="Price01" id="Price01" type="text"  size="10" value="<?=$Price01?>" maxlength="7" style="text-align:right" readonly> 원
            					<!-- 추후에 추가 -->
            					<!-- 
            					&nbsp;&nbsp;/&nbsp;&nbsp;
...
//취업률값에 따라 자부담금값 변경
function rateChange(){
	var EmpRate = $("#EmpRate").val(); //취업률
	var Price = $("#Price").val(); //교육비용
	var salePrice;
...
function SubmitOk() {
	...
	var Category1_length = $("input[name='Category1']").length;
	if(Category1_length>0) {
		for(i=0;i<Category1_length;i++) {
			if($("input[name='Category1']:eq("+i+")").is(":checked")==true) {
				if(Category1=="") {
					Category1 = $("input[name='Category1']:eq("+i+")").val();
				}else{
					Category1 = Category1 + "," + $("input[name='Category1']:eq("+i+")").val();
				}
			}
		}
	}
	var Category2_length = $("input[name='Category2']").length;
	if(Category2_length>0) {
		for(i=0;i<Category2_length;i++) {
			if($("input[name='Category2']:eq("+i+")").is(":checked")==true) {
				if(Category2=="") {
					Category2 = $("input[name='Category2']:eq("+i+")").val();
				}else{
					Category2 = Category2 + "," + $("input[name='Category2']:eq("+i+")").val();
				}
			}
		}
	}
	var Category3_length = $("input[name='Category3']").length;
	if(Category3_length>0) {
		for(i=0;i<Category3_length;i++) {
			if($("input[name='Category3']:eq("+i+")").is(":checked")==true) {
				if(Category3=="") {
					Category3 = $("input[name='Category3']:eq("+i+")").val();
				}else{
					Category3 = Category3 + "," + $("input[name='Category3']:eq("+i+")").val();
				}
			}
		}
	}
	$("#CategoryA").val(Category1);
	$("#CategoryB").val(Category2);
	$("#CategoryC").val(Category3);
	...
	if($('input[name="Category1"]').is(":checked") == false){
	...
	if($("#Price01View").val()=="") {
		alert("교육비용:일반훈련생을 입력하세요.");
		$("#Price01View").focus();
		return;
	}
	if(IsNumber($("#Price01View").val())==false) {
		alert("교육비용:일반훈련생은 숫자만 입력하세요.");
		$("#Price01View").focus();
		return;
	}
	if($("#Price02View").val()=="") {
		alert("교육비용:취업성공패키지를 입력하세요.");
		$("#Price02View").focus();
		return;
	}
	if(IsNumber($("#Price02View").val())==false) {
		alert("교육비용:취업성공패키지는 숫자만 입력하세요.");
		$("#Price02View").focus();
		return;
	}
	if($("#Price03View").val()=="") {
		alert("교육비용:근로장려금을 입력하세요.");
		$("#Price03View").focus();
		return;
	}
	if(IsNumber($("#Price03View").val())==false) {
		alert("교육비용:근로장려금은 숫자만 입력하세요.");
		$("#Price03View").focus();
		return;
	}
	if($("#Price01").val()=="") {
		alert("자부담금:일반훈련생을 입력하세요.");
		$("#Price01").focus();
		return;
	}
	if(IsNumber($("#Price01").val())==false) {
		alert("자부담금:일반훈련생은 숫자만 입력하세요.");
		$("#Price01").focus();
		return;
	}
	/*
	if($("#Price02").val()=="") {
		alert("자부담금:취업성공패키지를 입력하세요.");
		$("#Price02").focus();
		return;
	}
	if(IsNumber($("#Price02").val())==false) {
		alert("자부담금:취업성공패키지는 숫자만 입력하세요.");
		$("#Price02").focus();
		return;
	}
	if($("#Price03").val()=="") {
		alert("자부담금:근로장려금을 입력하세요.");
		$("#Price03").focus();
		return;
	}
	if(IsNumber($("#Price03").val())==false) {
		alert("자부담금:근로장려금은 숫자만 입력하세요.");
		$("#Price03").focus();
		return;
	}
	*/
