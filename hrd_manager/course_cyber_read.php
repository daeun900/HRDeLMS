...
?>
        ...
        <div class="contentBody">
            ...
            <div class="conZone">
            	<!-- ## START -->
<?
$Sql = "SELECT * FROM CourseCyber WHERE idx=$idx AND Del='N'";
...
//-->
				...
				<form name="DeleteForm" method="post" action="course_cyber_script.php" target="ScriptFrame">
                ...
                <table width="100%" cellpadding="0" cellspacing="0" class="view_ty01">
                  <colgroup>
					...
					<tr>
						...
						<td align="left"colspan="3">
							<? while (list($key,$value)=each($ContentsCategory1_array)){?>
						 	<input type="checkbox" name="Category1" id="Category1" value="<?=$key?>" <?if(in_array($key,$Category1_array)){?>checked<?}?> disabled> <label for="Category1"><?=$value?></label>&nbsp;&nbsp;&nbsp;&nbsp;
					 		<?}?>
					 		<? if($Category3){
					 		    echo "(";
                                while (list($key,$value)=each($ContentsCategory3_array)){
                            ?>
                            <input type="checkbox" name="Category3" id="Category3" value="<?=$key?>" <?if(in_array($key,$Category3_array)){?>checked<?}?> disabled> <label for="Category3"><?=$value?></label>&nbsp;&nbsp;&nbsp;&nbsp;
                            <?
					 		    }
					 		    echo ")";
					 		}
						...
						<th>과정 중분류</th>
						<td align="left"colspan="3">
							<? while (list($key,$value)=each($ContentsCategory2_array)){?>
						 	<input type="checkbox" name="Category2" id="Category2" value="<?=$key?>" <?if(in_array($key,$Category2_array)){?>checked<?}?> disabled> <label for="Category2"><?=$value?></label>&nbsp;&nbsp;&nbsp;&nbsp;
							<?}?>
						</td>
					</tr>
					<tr>
						<th>키워드(과정 소분류)</th>
						<td align="left" colspan="7"> <?=$Keyword?></td>
					</tr>
					<tr>
						<th>서비스 구분</th>
						...
						<th>진도체크방식</th>
						<td align="left" colspan="3"> <?=$ProgressCheck_array[$ProgressCheck]?></td>
					</tr>
					<tr>
						<th>취업률</th>
					...
					<tr>
						<th>자부담금</th>
						<td align="left" colspan="7"> 
    						<?=$CompanyScaleTitle01?> : <?=number_format($Price01,0)?> 원
    						<!-- 추후에 추가 -->
    						<!-- 
    						&nbsp;&nbsp;/&nbsp;&nbsp;
    						<?=$CompanyScaleTitle02?> : <?=number_format($Price02,0)?> 원&nbsp;&nbsp;/&nbsp;&nbsp;
    						<?=$CompanyScaleTitle03?> : <?=number_format($Price03,0)?> 원
    						 -->
						</td>
					</tr>
					<tr>
