...
$sql4select = "SELECT c.ContentsName, c.attachFile, c.Professor, c.CompleteTime, c.ChapterLimit, c.ContentsURLSelect AS ContentsURLSelectGlobal, 
                		...
                		cd2.Seq AS ContentsDetailSeq, cd2.ContentsType, cd2.ContentsURLSelect, cd2.ContentsURL, cd2.ContentsURL2,
                        cd2.MobileURL, cd2.ContentsMobilePage, cd2.Caption
                FROM Course c 
...
//         }
//     }
    $mobileURL = html_quote($MobileURL);
    $playPath = $MovieServerURL.$mobileURL;
    
    $contentsMobilePage = html_quote($ContentsMobilePage); //모바일 페이지수
...
//     $response['chapterLimit'] = $ChapterLimit; //차시제한 여부
    ...
    $response['contentsMobilePage'] = $contentsMobilePage; //pc 링크일 경우에는 미사용 
