...
foreach ($send_id_arr as $arr) {
    ...
    $seq = $send_arr[1];
    $id = $send_arr[2];
    $lecture_code = $send_arr[3];
    $class_agent_pk_arr = explode(",",$send_arr[4]);
    $lecture_terme_idx = $class_agent_pk_arr[1];
    $progressRate = $send_arr[5];
    $totalScore = $send_arr[6];
    $pass_flag = $send_arr[7];
    $changestate = $send_arr[10];
    $input_date = trim($send_arr[11]);
    if (strlen($input_date) > 0 && strlen($input_date) != 19) {
        echo "$input_date 날짜형식오류";
        exit;
    }
    if (strlen($pass_flag) > 1) {
        echo "$pass_flag pass_flag오류";
//        exit;
    }
...
// 전체 회원
//$send_id_arr=getList('HRDLMS.Member',"",""); 
//foreach ($send_id_arr["list"] as $row) {
//    $id = $row["ID"];
//    $name = $row["Name"];

    $data=getList('hrd.Study',"ID='{$id}' AND (ServiceType=1 OR ServiceType=4) AND LectureCode='$lecture_code' AND LectureTerme_idx='$lecture_terme_idx' AND LectureStart>='2022-08-02' ",5000,"","Seq desc");
    if (!count($data["list"])) {
        $IP=getList('hrd.Study',"ID='{$id}' ",1);
