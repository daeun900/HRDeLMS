...
//if($ServiceType==100 && $MobileAuth_count<1 && ($ServiceType==1 || $ServiceType==4)) { //과정 인증내역이 없으면 본인인증 필요 (과정당 1회만 인증) 입과시 인증
if( $MobileAuth_count<1 && ($ServiceType==1 || $ServiceType==4)) { //과정 인증내역이 없으면 본인인증 필요 (과정당 1회만 인증) 입과시 인증
	...
	if($ServiceType==1 || $ServiceType==4){	//환급과정 체크
	    if(empty($_SESSION['PlayStudyAuth_'.$Study_Seq.$Chapter_Seq])){	//해당 차시를 인증 안한경우
			...
			//1,9,17...차시인경우
            //241031 8차시 본인인증 -> 1일1회 본인인증 변경으로 인한 주석처리
...
//241002.YEON. 고문님께서 진도시간기준 기능이 환급과정에서도 적용되어야한다고 하셔서 주석처리 진행함.
//241010.YEON. 진도율이슈로인해 다시 주석해제
